...
def milisec_converter(mili_sec):
    ...
    second = str(round((float(format(minute, '.4f')) -
                        (int(mili_sec)//60000)) * 60, 2)).replace('.', '\'')
...
class CommentView(View):
    def get(self, request, user_id):
        try:
            if GameUser.objects.filter(access_id=user_id).exists():
                ...
                comments = Comment.objects.filter(to_from_id=user).values()
    ...
    def post(self, request, user_id):
        try:
            ...
            from_user = request.userinfo.game_user.id
            to_user = GameUser.objects.get(access_id=user_id).id

            Comment(
                comment=data['comment'],
                from_to_id=from_user,
                to_from_id=to_user
            ).save()
...
class RankDetailView(View):
    def get(self, request, access_id):
        try:
            ...
            gameuser = GameUser.objects.select_related(
                'userpagehit', 'detail').get(access_id=access_id)
...
class IndiDetailTrackView(View):
    def get(self, request, access_id):
        ...
        info_exists_in_records = [
            i for i in track_infos if i.track in records_exists]
        track_info_result = [
            {
                'play_cnt': i.play_cnt,
...
class IndiDetailTrackDist(View):
    def get(self, request, access_id, track_key):
        ...
        track_record3 = {str(milisec_converter(i)): j for i, j in eval(
            track_record2.cumul_dist)[1].items()}
        track_record4 = [str(milisec_converter(
            eval(track_record2.cumul_dist)[0])), track_record3]
        track_record_result = [
            {
...
class IndiRankListView(View):
    def get(self, request):
        ...
        rank_list = Ranking.objects.prefetch_related(
            'game_user_set').filter(team_type_id=team_id).values()
